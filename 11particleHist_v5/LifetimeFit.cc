...
LifetimeFit::LifetimeFit( float min1, float max1, 
 ...
 min(min1), max(max1),
 minTime(minTime1), maxTime(maxTime1),
 minScanRange(minScan1), maxScanRange(maxScan1), scanStep(scanStep1) { // initializations
}
...
bool LifetimeFit::add( const Event& ev ) {
  ...
  double PT;
  PT = ProperTime::instance()->decayTime();
  // check for mass being in range
  if( M>min && M<max) {
	DecayTimes.push_back(PT);
...
void LifetimeFit::compute() {
  ...
  PMLifetimeError = 1.0/sqrt(2*qf.c());
